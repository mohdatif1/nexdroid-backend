...
// ─── KEYSTORE: GET EXISTING OR MARK FOR GENERATION ────────
// Pehli build: keystoreBase64 = null → GitHub Actions pe keytool se generate hoga
// Baad ki builds: Firestore se saved keystore milegi → wahi use hogi (same signature)
async function getOrCreateKeystore(packageName, ksParams) {
  ...
  if (ksDoc.exists) {
    // Existing keystore mili — wahi return karo (signature same rahegi)
    const d = ksDoc.data();
  ...
  // Pehli build — keytool server pe nahi chalega
  // keystoreBase64 = null bhejo → generateWorkflow() GitHub Actions pe keytool chalayega
  // Build complete hone ke baad saveKeystoreIfNew() keystore Firestore mein save karegi
  console.log(`[Keystore] New app — keystore will be generated by GitHub Actions for ${packageName}`);
  return {
    keystoreBase64: null,   // null = GitHub Actions pe keytool chalega
...
// ─── SAVE KEYSTORE AFTER FIRST BUILD ──────────────────────
// GitHub Actions ne keystore generate ki — pipeline ke end mein Firestore mein save karo
// (Next build pe same keystore milegi — signature consistent rahega)
async function saveKeystoreIfNew(packageName, ksResult, repoName) {
  if (!ksResult.isNew) return; // Already saved, skip

  try {
    // GitHub repo se generated keystore fetch karo
    const ksFile = await github.getFileContent(repoName, 'release.keystore');
    if (!ksFile) {
      console.warn(`[Keystore] Could not fetch keystore from repo ${repoName} — will regenerate next build`);
      return;
    }

    const db = getDB();
    await db.collection('keystores').doc(packageName).set({
      keystoreBase64: ksFile,   // base64 encoded keystore
    ...
    console.log(`[Keystore] New keystore saved to Firestore for ${packageName}`);
  } catch (e) {
    // Save fail hone pe build fail nahi hogi — next build pe phir generate hogi
    console.warn(`[Keystore] Failed to save keystore: ${e.message}`);
  }
}
...
async function runBuildPipeline(buildId, repoName, uid, config) {
  ...
  try {
    ...
    // Pehli build ke baad GitHub Actions ki generated keystore Firestore mein save karo
    // Taaki next build pe same keystore use ho (signature consistent rahe)
    await saveKeystoreIfNew(config.packageName, ksResult, repoName);
